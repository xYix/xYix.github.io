...
每新增一个等价类必定要合并两个集合，这就证明了**引理 1.3**。

$\blacksquare$

另外我们还可以推出，同一个 endpos 等价类中的串互相有后缀关系且长度连续，记其最长者为 $\text{maxlen}()$，最短者为 $\text{minlen}()$。我们定义某个**等价类的后缀**为其最短串的后缀们属于的那些等价类。

- 转移边的定义是显然的，只需要注意到：同一个等价类的串末尾同时加某个字符得到的一组串，它们仍属于同一个等价类。
- 为了符合要求的性质，后缀自动机上跳 $\text{fail}$ 应当可以遍历一个串的所有后缀（属于的等价类），从而某等价类的 $\text{fail}$ 边应该指向该等价类的最长后缀。下面列出一些有关 $\text{fail}$ 的事实。
  ...
  - > $\text{minlen}(u)=\text{maxlen}(\text{fail}(u))+1$。
    ...
    >
    > 为了方便，我们有时按 $\text{fail}$ 边把 SAM 看成一个树形结构，称为**parent 树**。一个等价类的后缀也可以认为是其 parent 树上的父亲。祖先和后缀是同义词。

于是我们便明确了节点、转移边和 $\text{fail}$ 边的定义。

接下来我们如下构建 SAM：每当增加一个字符，记新插入的字符为 $\mathbf c$，当前长度为 $i$。

- 显然 $\{i\}$ 当然是一种新出现的 endpos。
- $\{i-1\}$ 的祖先（再次提醒：祖先和后缀是同义词）中有这样一些等价类 $p$，它们足够稀有，以至于 $p+(\mathbf c)$ 从未出现（$p$ 没有 $\mathbf c$ 的转移边），于是在插入 $i$ 后 $p+(\mathbf c)$ 一定是 $\{i\}$ 的子集，我们直接连边 $p\xrightarrow{\mathbf c}\{i\}$。显然 $p+(\mathbf c)$ 并不是 $\text{fail}(\{i\})$ 的子集：它已经是 $\{i\}$ 的子集了。
- 令最长（等价于在 parent 树上最深）的，$\xrightarrow{\mathbf c}$ 存在的 $\{i-1\}$ 的后缀为 $p$。记 $q$ 为这条转移边指向的节点：$p\xrightarrow{\mathbf c}q$。
- - 很明显，$\text{fail}(\{i\})$ 就在 $q$ 中：$q$ 中的确有 $\{i-1\}$ 的后缀 $+(\mathbf c)$，也就是 $\{i\}$ 的后缀 ；且其 endpos 不是 $\{i\}$，而更长的那些后缀都太稀有，endpos 是 $\{i\}$。
  - 但是注意用词，$q$ 中的确有不代表全是。长度在 $[\text{minlen}(p)+1,\text{maxlen}(p)+1]$ 的那些串才的确是 $p+(\mathbf c)$，会在 $i$ 处出现。当然更短的也行。更长的就不行了，它们不在 $i$ 处出现，应当剔除。
  - 从而我们把 $q$ 分裂为两个点 $q1$ 和 $q2$ 即可。想必转移边和 $\text{fail}$ 的维护已经是不需要陈述的细节了。
