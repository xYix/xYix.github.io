...
| AGC031F | [Walk on Graph](https://atcoder.jp/contests/agc031/tasks/agc031_f) | $\color{violet}\text{pigeon}$  |     是     |
...
| AGC034E | [Complete Compress](https://atcoder.jp/contests/agc034/tasks/agc034_e) |                                |            |
...
有些题题解写了一半发觉好像实在没有什么写的必要就鸽了。给这种 ad-hoc 鬼题写题解真的对解其他题一点用都没有……
...
下面我们用 $(u,w)$ 表示下面这个状态：目前站在 $u$，权值为 $w$。"一个状态能走到另一个状态"这个关系记为 $\Rightarrow$。

**观察 1.** 考虑反复走同一条边，则权值会依次变为 $w,2w+d,4w+3d,8w+7d,...$，又必然存在 $2^k=1\pmod p$，即走 $k$ 步又会走回 $w$。即使奇偶性有问题，再走 $k$ 步就能调整回来。
...
$$
由于 $2$ 必有逆元，即 $(v,w)\Leftrightarrow(v,w+2l_u)$。即 $l_v$ 被更新为 $\gcd(l_v,2l_u)$。又循环节必定是 $p$ 的约数，即也是奇数，故 $l_v\leftarrow\gcd(l_u,l_v)$。最终的稳定状态必定满足 $l_v|l_u,l_u|l_v$，又图连通，故最终所有点的 $l$ 总是相等，且等于 $\gcd_{u,a,b}(3a-3b)=3\text{gcd}_{u,a,b}(a-b)$，记为 $3g$。

我们实际上证明了 $(u,w)\Leftrightarrow(u,w+3g)$，但是实际上的 $l$ 应当是 $\gcd(3g,p)$。那么我们不如令 $p\leftarrow \gcd(3g,p)$。
...
考虑 $a,b,c$ 必须顺序出现的情形，可见最终的 $G$（记为 $G_M$）中有边 $(x,y)$ 当且仅当存在序列 $x,u_1,...,u_{\ell},y$ 使得它们在 $T$ 上的一条链上恰好顺序出现，且 $(x,u_1),(u_1,u_2),...$ 等皆存在于 $G$ 中。直接在 $G$ 上从每个点开始搜即可。

$a,b,c$ 必须顺序出现的情形的性质的确很好，现在考虑从 $a,b,c$ 不必顺序出现向它转化。

考虑 $T$ 上顺序出现的三个点 $a,b,c$，若 $(a,b),(b,c),(a,c)$ 中有两条边存在，则第三条也必定存在，换言之 $u_{i-1},u_i,u_{i+1}$ 是可以任意排列的，易知整个序列也就是可以任意排列的。于是，只要我们手动把第三条边补齐，就一定可以按“$a,b,c$ 必须顺序出现”做。更精确地，我们其实只需要把 $(a,b),(a,c)$ 转化为 $(a,b),(b,c)$。
