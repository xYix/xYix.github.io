...
void init() 
{
  n = sc.n(); m = sc.n(); k = sc.n(); b = sc.n();
  for (int i = 0; i < n; i++) sc.next(s[i]);
} 

std::vector<int> e[N * 2];
int col[N * 2], bpcnt[N * 2], becnt[N * 2], bkcnt[N * 2], pikcnt[N * 2];
void dfs(int u, int cid)
{
  col[u] = cid; bpcnt[cid]++;
  for (int v: e[u]) if (col[v] == -1) dfs(v, cid);
}
...
void solve() 
{
  ...
  for (int i = 0; i < cid; i++)
    if (becnt[i] >= bpcnt[i])
      checkMax(ans, bkcnt[i]);
  for (int i = 0; i < n + m; i++) 
    if (col[i] != -1 && e[i].size() == 1)
    {
      // see(i, col[i], bkcnt[col[i]], pikcnt[i]);
      checkMax(ans, bkcnt[col[i]] - pikcnt[i]);
    }
