...
#ifdef __LOCALE__
    if (*buffPos == '-') { fprintf(stderr, "warning: Scanner skip '-' while reading unsigned integer\n"); }
#endif
      buffPos++;
      if (buffPos == buffLim) {
        flushBuff(); } }
...
void sp_ori(std::vector<int> a)
{
  if (a.size() <= 1) return;
  else if (a.size() <= 3) { puts("no"); exit(0); }
  
  std::vector<int> b(a.begin(), a.end() - 4);
  sp_ori(b);

  auto push = [&](int i, int j) 
  { 
    ops.emplace_back(a[i], a[j]); 
    std::swap(a[i], a[j]);
  };
  auto mover2l = [&](int l, int r, int p)
  {
    assert(r + 1 == p);
    for (int i = r; i >= l; i--) push(i, i + 1);
  };
  auto movel2r = [&](int p, int l, int r)
  {
    assert(p + 1 == l);
    for (int i = l; i <= r; i++) push(i, i - 1);
  };

  const int l = (int)(a.size()) - 5;
  mover2l(0, l, l+1);
  mover2l(1, l+1, l+2);
  push(0, 1);
  push(1, l+3);
...
std::vector<std::pair<int,int>> gen_swaps(int n, int s, int t)
{
  assert(s != t);

  std::vector<std::pair<int,int>> res; int last = s;
  auto to = [&](int dest) { res.emplace_back(std::exchange(last, dest), dest); };
  for (int i = 0; i < n; i++) if (i != s && i != t) to(i); to(t);
  see(n, s, t, res);
  return  res;
}

void sort_to(std::vector<int> from, std::vector<int> to)
{
  see(from, to);
  const int n = static_cast<int>(from.size());
  if (n == 2)
  {
    if (from[0] == to[0]) { puts("no"); exit(0); }
    ops.emplace_back(from[0], from[1]);
    return;
  }
  ...
  int diff_to = static_cast<int>(std::find(to.begin(), to.end(), from[diff_from]) - to.begin());
  auto swaps = gen_swaps(n, diff_from, diff_to);
  see(n, swaps);

  std::reverse(swaps.begin(), swaps.end());
  for (const auto& [x,y] : swaps) { std::swap(to[x], to[y]); }

  auto v_from = from; v_from.erase(v_from.begin() + diff_from);
  auto v_to = to; v_to.erase(v_to.begin() + diff_from);
  sort_to(v_from, v_to);

  std::reverse(swaps.begin(), swaps.end());
  for (const auto& [x,y] : swaps) { ops.emplace_back(to[x], to[y]); std::swap(to[x], to[y]); }
}
