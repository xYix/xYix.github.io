...
void part1()
{
  ...
  while (!odd.empty())
  {
    int u = *odd.begin(); odd.erase(odd.begin());
    ops.emplace_back(1, u);
    cnt[u] = 0;
    for (int v: e[u]) if (cnt[v]) {
      if (cnt[v] % 2 == 1) { odd.erase(v); } cnt[v]--;
      if (cnt[v] % 2 == 1) { odd.insert(v); } }
    e[u].clear();
  }
}
...
void dfs(int u, int f)
{
  ...
  int up_v = -1;
  for (int v: e[u]) if (cnt[v] && v != f)
  {
    if (dep[v] == -1) dfs(v, u);
    else if (dep[v] < dep[u]) up_v = v;
  }
  if (up_v != -1)
    ups.emplace_back(u, up_v);
}

void part2()
{
  memset(dep, -1, sizeof(dep)); dep[0] = 0;
  for (int u = 1; u <= n; u++) if (dep[u] == -1)
  {
    if (cnt[u]) 
    {
      ups.clear();
      dfs(u, 0);
      std::sort(ups.begin(), ups.end(), [](auto x, auto y) { return dep[x.second] > dep[y.second]; });
      for (const auto& [u, t] : ups)
      {
        int det = 0;
        int p = u;
        while (p != t)
        {
          ops.emplace_back(1, p + det);
          det = n - det;
          if (fa[p] == t) ops.emplace_back(1, p + det);
          p = fa[p];
        }
        ops.emplace_back(1, u + n);
      }
    }
    ops.emplace_back(1, u);
  }
